...
当前版本 `0.4.0.dev0`：阶段一模块 0～9、阶段二模块 0～12、阶段三模块 0～12 已完成；阶段四已实现模块 0～7，PostgreSQL、容器与真实模型效果验收待补。新增固定维度向量索引、混合检索、实际注入选择冻结，以及可显式开启的记忆和归档召回。
...
记忆提议、确认和归档操作见[模块 3～5 运行说明](docs/阶段四-模块3至5验收与运行说明.md)。MCP、Redis、独立执行容器与 Memory 管理前端仍属后续模块。
