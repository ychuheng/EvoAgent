...
当前版本 `0.4.0.dev0`：阶段一模块 0～9、阶段二模块 0～12、阶段三模块 0～12 已完成；阶段四已实现模块 0～8，PostgreSQL、容器与真实模型效果验收待补。已具备向量索引、混合检索、上下文选择冻结，以及 MCP 受控连接、工具发现、目录版本和本地审核元数据。
...
记忆提议、确认和归档操作见[模块 3～5 运行说明](docs/阶段四-模块3至5验收与运行说明.md)。MCP 工具执行适配、Redis、独立执行容器与 Memory 管理前端仍属后续模块。
