...
> 补完说明（2026-09-25）：上述“模块已实现”是工程清单状态。用户从网页观察审批、取消、失败、Worker 重启与恢复，并核对页面状态和 Trace 的联合验收仍待完成；执行顺序及门槛见[前四阶段补完与用户场景验收计划](前四阶段补完与用户场景验收计划.md)。原故障注入测试继续有效，但不能替代这一用户场景。
