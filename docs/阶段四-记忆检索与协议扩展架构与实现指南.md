...
>
> 当前状态（2026-09-21）：已实现模块 0～10，版本 `0.4.0.dev0`；模块 11～14 仍为计划。模块 10 新增独立 controller、固定禁网容器、租约清理、Artifact 发布及网页 IP 绑定，并已在 Docker Desktop Linux Engine 完成 11 项真实容器验收；真实 PostgreSQL/pgvector、迁移和应用容器补验也已通过，真实模型效果证据仍待补。详见[模块 10 验收说明](阶段四-模块10验收与运行说明.md)、[ADR-012](ADR-012-独立执行容器与受控网络出口.md)和[开发进度](开发进度与决策记录.md)。
>
