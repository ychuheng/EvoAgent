...
>
> 当前状态（2026-09-20）：已实现模块 0～7，版本 `0.4.0.dev0`；模块 8～14 仍为计划。新增固定 1536 维索引、generation 重建、BM25/RRF 混合检索及零命中在内的上下文选择冻结。默认保留原词法 Skill 检索，Memory/Archive 注入显式启用，候选仍需人工确认。PostgreSQL+vector、容器及真实模型效果证据仍待补。详见[模块 6～7 验收说明](阶段四-模块6至7验收与运行说明.md)、[ADR-009](ADR-009-派生向量索引与检索冻结.md)和[开发进度](开发进度与决策记录.md)。
>
...
> 本节保留编写指南时的阶段三基线，用于说明阶段四的出发点；模块 0～7 已完成的改动以文首状态、第 28 节逐模块记录和最新验收说明为准。
...
用户随后授权继续实现模块 3～7，现已补齐相应代码、测试与详细源码手册。后续实现单元是模块 8；PostgreSQL+vector、容器和真实效果报告仍是独立验收待办，不能由本地 Mock/SQLite 测试替代。
