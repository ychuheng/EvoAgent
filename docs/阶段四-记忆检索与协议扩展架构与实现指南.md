...
>
> 当前状态（2026-09-21）：已实现模块 0～10 的代码及本地契约验收，版本 `0.4.0.dev0`；模块 11～14 仍为计划。模块 10 新增独立 controller、固定禁网容器、租约清理、Artifact 发布及网页 IP 绑定；Linux Docker、PostgreSQL+vector 和真实模型效果证据仍待补。详见[模块 10 验收说明](阶段四-模块10验收与运行说明.md)、[ADR-012](ADR-012-独立执行容器与受控网络出口.md)和[开发进度](开发进度与决策记录.md)。
>
...
**实现状态（2026-09-21）：代码与本地契约验收完成，真实 Linux Docker 隔离验收待补。** 新增 `sandbox/`、migration `20260921_0010`、部署 overlay 和 Docker CI 任务。Shell 无宿主 fallback，第三方 stdio 使用固定容器 Profile；所有执行容器仅支持 network=none，联网需求通过 Worker 的受控 web_fetch 获取资料后显式输入。未实现通用联网容器网关、容器密钥注入或多 controller。源码详解见手册第 82～85 章，执行与证据边界见[模块 10 运行说明](阶段四-模块10验收与运行说明.md)。下列目标中的实机测试必须另行获得日志后验收。
