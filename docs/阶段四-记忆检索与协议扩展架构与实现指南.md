...
>
> 当前状态（2026-09-22）：模块 0～13 已有工程实现，版本 `0.4.0.dev0`；模块 14 工程收尾已交付，真实 Embedding 语义验收仍为 pending，正式 v0.4 门禁未放行。最新证据见[模块 14 说明](阶段四-模块14最终验收与交付说明.md)。Memory/MCP/Context 三个新页面和原三个浏览器流程回归通过，详见[模块 13 验收说明](阶段四-模块13验收与运行说明.md)。模块 11 的 Redis 唤醒/限流、独立维护 Worker、Eval epoch 和真实双进程已验收；模块 12 新增隔离单变量实验、固定数据集、API/CLI、Mock 与真实模型分开的报告。真实模型小样本不替代完整效果与真实 Embedding 验收。详见[模块 11～12 验收说明](阶段四-模块11至12验收与运行说明.md)、[ADR-013](ADR-013-数据库持久队列与Redis唤醒限流.md)、[ADR-014](ADR-014-独立运行时实验与隔离评测.md)和[开发进度](开发进度与决策记录.md)。
>
...
> 补完说明（2026-09-25）：网页基本对话虽已接通，长上下文、记忆、MCP 和多 Worker 的用户任务链路仍须联合验收；真实记忆小样本未显示收益，4K bounded 小样本曾在请求前拒绝，真实 Embedding 未验收。按[前四阶段补完与用户场景验收计划](前四阶段补完与用户场景验收计划.md)补验并保留失败证据；不能把模块 14 工程收尾当成正式 v0.4 放行。
...
**实现状态（2026-09-21）：代码与本地协议回归完成。** `mcp/adapter.py`、`bindings.py` 接通 BaseTool/Registry/Executor；`runs.tool_catalog_snapshot` 保存包含空选择的完整契约；execution 状态独立 CAS、审核身份与 policy_hash 绑定审批，稳定语义键避免换目录绕过未决写入。只读最多重试一次，所有写默认单次，文本/结构化结果与长正文 Artifact 沿用统一链路；draining 等在途结束，disabled 取消并保持不确定写 UNKNOWN。迁移为 `20260921_0009`。不扩大 Skill 白名单，不污染 baseline/pinned 目录。详见[手册第 78～81 章](EvoAgent-源码讲解与学习手册.md)和[运行说明](阶段四-模块9验收与运行说明.md)。
...
**实现状态（2026-09-21）：代码、本地契约和 Docker Desktop Linux Engine 实机验收完成。** 新增 `sandbox/`、migration `20260921_0010`、部署 overlay 和 Docker CI 任务。Shell 无宿主 fallback，第三方 stdio 使用固定容器 Profile；所有执行容器仅支持 network=none，联网需求通过 Worker 的受控 web_fetch 获取资料后显式输入。11 项真实容器测试已验证 cgroup、耗尽、进程树清理、禁网及特殊文件拒绝。未实现通用联网容器网关、容器密钥注入、多 controller 或敌对多租户强隔离。源码详解见手册第 82～85 章，执行与证据边界见[模块 10 运行说明](阶段四-模块10验收与运行说明.md)。
...
实现状态（2026-09-22）：已完成。新增三个页面、阶段四 API types/client 和只读 Context 证据接口；组件 12 passed、浏览器 6 passed，全量后端 404 passed、13 skipped。Memory 来源/冲突/人工决定/删除 Job，MCP 目录审核/健康/排空/卸载，上下文预算/修订/检索证据均已接入。运行方式和明确边界见[模块 13 验收说明](阶段四-模块13验收与运行说明.md)，源码讲解见手册第 94～97 章。
...
实现状态（2026-09-23）：工程交付完成，四条 Demo/迁移/容器/页面/阶段三回归和真实算术 Skill 配对报告已落库并导出；真实 Embedding 语义验证缺配置，正式完成门禁仍为 pending。详见[交付矩阵](阶段四-模块14最终验收与交付说明.md)、[ADR-015](ADR-015-阶段四交付证据与发布门禁.md)和手册第 98～101 章。
...
- [x] v0.3 基线复核完成，真实模型报告、PostgreSQL 和容器证据位置明确。
- [x] Worker 主循环能自动处理 RECOVERING，未决副作用得到正确处置。
- [x] 每次请求发送前检查完整输入与输出预留；估算和精确计数明确区分。
- [x] 不能压缩时明确停止；不会破坏工具消息配对或删掉任务关键约束。
- [x] ContextRevision 和快照可恢复，压缩不重置预算或调用保护。
- [x] Session 输入/答案持久化、序号与历史截止点能支持确定性历史读取。
- [x] Memory 具备版本、来源、作用域、确认、冲突、过期、撤销和内容清理流程。
- [x] 跨 scope、过期、撤销内容不能通过词法、向量、缓存或旧快照重新注入。
- [x] pgvector 在真实 PostgreSQL 验证；profile 切换、失效和重建不会混用向量。
- [x] Skill/Memory 混合检索可解释、可降级、可冻结，无命中也是持久事实。
- [x] MCP stdio/HTTP 可发现、审核启用、调用和卸载，未知 Schema 明确拒绝。
- [x] MCP 调用经过 Executor/Policy/Approval/Effect，不信任远端风险自述。
- [x] 目录变更不会静默改变在途 Run；旧审批不能授权新工具契约。
- [x] 双 Worker 真实竞争通过，过期执行者不能提交事件、快照、效果或终态。
- [x] Redis 故障不丢数据库任务，限流故障不自动放开全局外部调用。
- [x] Docker 的资源/时间/输出/网络限制及取消清理通过真实容器测试。
- [x] Skill 配对的唯一变量与 TRAIN/HOLDOUT 隔离继续成立。
- [x] 压缩、记忆和检索分别有冻结实验，保留失败、未知成本和不可比较样本。
- [x] Memory/MCP 管理页与原 Skill/Eval 页面回归通过（模块 13，新增 3 个/原 3 个浏览器流程；HTTP fixture 与真实后端 API 测试分开记录）。
- [x] 新安装、旧库升级、关闭新能力、四个 Demo 和安全限制均有准确文档。

以上勾选仅对应模块 14 交付矩阵中的明确工程/报告范围，Mock、SQLite、真实环境和小样本区别见交付说明。

- [ ] 真实 Embedding 语义服务与固定数据集对照验收（当前缺配置，正式发布门禁 pending）。
...
用户随后授权继续实现模块 3～9，现已补齐相应代码、测试与详细源码手册。后续实现单元是模块 10；PostgreSQL+vector、容器和真实效果报告仍是独立验收待办，不能由本地协议 fixture 或 Mock/SQLite 测试替代。
