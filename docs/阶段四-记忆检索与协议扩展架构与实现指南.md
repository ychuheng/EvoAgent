...
>
> 当前状态（2026-09-21）：已实现模块 0～9，版本 `0.4.0.dev0`；模块 10～14 仍为计划。模块 9 新增 MCP 适配器、动态 Schema 校验、显式目录激活、Run 冻结、审批绑定、UNKNOWN 和卸载。PostgreSQL+vector、容器及真实模型效果证据仍待补。详见[模块 9 验收说明](阶段四-模块9验收与运行说明.md)、[ADR-011](ADR-011-MCP工具契约冻结与统一安全执行.md)和[开发进度](开发进度与决策记录.md)。
>
