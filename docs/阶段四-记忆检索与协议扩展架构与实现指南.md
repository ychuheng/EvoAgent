...
>
> 当前状态（2026-09-22）：模块 0～12 已有工程实现，版本 `0.4.0.dev0`；模块 13～14 仍为计划。模块 11 的 Redis 唤醒/限流、独立维护 Worker、Eval epoch 和真实双进程已验收；模块 12 新增隔离单变量实验、固定数据集、API/CLI、Mock 与真实模型分开的报告。真实模型小样本不替代完整效果与真实 Embedding 验收。详见[模块 11～12 验收说明](阶段四-模块11至12验收与运行说明.md)、[ADR-013](ADR-013-数据库持久队列与Redis唤醒限流.md)、[ADR-014](ADR-014-独立运行时实验与隔离评测.md)和[开发进度](开发进度与决策记录.md)。
>
