...
>
> 本文保留当时的设计和学习路线，具体实现代码、后续演进及最新状态以仓库源码、学习手册和开发进度为准。

> 补完说明（2026-09-25）：本指南的模块验收证明 Agent 内核可测试，不证明网页真实模型与工具任务已由用户完整验收。当前网页基本对话可用；真实模型从页面选择并连续调用工具、结果展示、取消/超时等用户场景仍按[前四阶段补完与用户场景验收计划](前四阶段补完与用户场景验收计划.md)补验。下文原阶段一范围保持历史设计，不以此次补验反写为当时已完成。
